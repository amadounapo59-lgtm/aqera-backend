...
import { structuredLog } from '../logger/structured.logger';
...
export class AllExceptionsFilter implements ExceptionFilter {
    ...
    // 5) Structured log (JSON one line) for aggregators — skip error log for non-API 404s (e.g. browser requesting /_next/*, favicons)
    const pathStr = (req?.url ?? '').split('?')[0];
    const isNonApi404 =
      status === 404 &&
      (pathStr.startsWith('/_next') ||
        pathStr === '/favicon.ico' ||
        pathStr.startsWith('/__nextjs') ||
        pathStr === '/apple-touch-icon.png' ||
        pathStr === '/apple-touch-icon-precomposed.png' ||
        /^\/apple-touch-icon.*\.png$/i.test(pathStr));
    if (isNonApi404) {
      structuredLog.debug('http_404_non_api', { path: req?.url, method: req?.method });
    } else {
      structuredLog.error('http_error', {
        status,
        path: req?.url,
        method: req?.method,
        message,
        ...
        raw: exception?.message ?? undefined,
      });
    }
